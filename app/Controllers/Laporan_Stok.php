	...
	public function index() {
		if(isset($_GET['filter']) && ! empty($_GET['filter'])){ // Cek apakah user telah memilih filter dan klik tombol tampilkan
			...
			$data['detail_transaksi'] = $detail_transaksi_model->select(' *,transaksi.id as id, detail_transaksi.id_transaksi as id_detail, detail_transaksi.harga as harga, detail_transaksi.jumlah as jumlah, nama_menu, id_menu')
				...
				->where($where )
				->findAll();
			$data['transaksi'] = $transaksi_model->select('date_format(tanggal,"%d-%m-%Y") as tanggal, str_to_date(tanggal, "%d-%b-%Y") as orderbytanggal, total_harga,id_user,id_menu,id_transaksi, count(1) as count')->join('detail_transaksi', 'transaksi.id= detail_transaksi.id_transaksi')
				// ->where($filter_column, $filter_var )
				->where($where )
				->orderBy( orderbytanggal,'desc')
